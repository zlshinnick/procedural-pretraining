"""Delete task dataset for sequence manipulation training.

This module provides a procedurally generated dataset where the model must
predict a sequence with one specified token removed.
"""
...
class DeleteDataset(Dataset):
    """Procedurally generated dataset for the delete task.

    Each sample is a random sequence of tokens, and the target is the sequence
    with one specified token removed.
    ...
    def __init__(
        ...
        """Initializes the delete task dataset.
    ...
    def _generate_delete_sequence(
        self,
        seed: int,
    ) -> tuple[Tensor, Tensor, Tensor]:
        """Generates a delete sequence.
        ...
        removed_token = local_rng.randint(1, self.vocab_size, size=1).tolist()

        result = []
        removed = False
        for token in sequence:
            if token == removed_token[0] and not removed:
                removed = True
            else:
                result.append(token)
        ...
        if len(result) < self.seq_len:
            result += [self.pad_token] * (self.seq_len - len(result))
        ...
        if sep_idx >= 0:
            for i in range(sep_idx, sep_idx + len(result) + 1):
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        """Retrieves a single sample for the delete task.
        ...
        """
        sequence, target, loss_mask = self._generate_delete_sequence(self.seed + idx)
...
def build_dataloader(
    ...
    """Builds train and validation dataloaders for the delete task dataset.
