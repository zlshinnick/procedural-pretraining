...
experiment_config_path = "downstream/algorithmic_tasks/configs/baseline.yaml"

# %% config from command line
exec(
    open("downstream/algorithmic_tasks/configurator.py").read()
)  # overrides from command line or config file
print(f"Task: {task}", flush=True)
print(f"Seed: {seed}", flush=True)

# Fix: make sure to strip any trailing commas from task name
task = task.strip(",")
...
experiment_config = ExperimentConfig.from_yaml(experiment_config_path)

try:
    exec(open("downstream/algorithmic_tasks/task_" + task_name + ".py").read())
except Exception as e:
    print(f"Error loading task {task_name}: {e}")
    raise  # Re-raise the exception instead of continuing

exec(open("downstream/algorithmic_tasks/configurator.py").read())  # re-override

# %% generate data
taskA_test = taskA_gen()
# assert everything is within vocab size
for d in taskA_test:
    assert all([0 <= x < vocab_size for x in d])
# shuffle
...
save_path = (
    task
    + "/"
    + (random_str + " " + task + " " + " " + run_description)
    .replace("(", "_")
    .replace(")", "")
    .replace(" ", "_")
    .replace(",", "_")
...
if arch == "transformer":
    ...
    model, num_trainable, num_frozen = initialize_model(
        gpt2_config=config,
        pretrained_model_path=experiment_config.pretrained_model_path,
        weights_to_transfer=experiment_config.weights_to_be_transferred,
        weights_to_train=experiment_config.weights_to_be_trained,
        embedding_init_strategy=experiment_config.embedding_init_strategy,
        device=device,
    )
...
datasets = {
    key: CustomDataset(tokenized_data=torch.tensor(val, dtype=torch.long))
    if val is not None and len(val)
    else CustomIterDataset()
...
class L2NormCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        l2_norm_noln = sum(
            [p.norm() ** 2 for n, p in model.named_parameters() if ".ln" not in n]
        ).sqrt()
